...
practical time and within available memory. When computing chains, 
cheddar allocates memory as required, with a safety limit to prevent too much 
of the system's memory from being consumed. 
