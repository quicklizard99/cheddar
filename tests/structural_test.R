...
TestNicheModelLinks <- function()
{
    ...
    # pool of 10 species.
    res <- NicheModelLinks(pool=paste('Node', 1:10), n=500, C=0.5)
    AssertEqual(500, length(res))
    ...
    # 100 sets of links with directed connectance of 0.5 taken from from a 
    # pool of 10 species.
    res <- NicheModelLinks(pool=paste('Node', 1:10), n=500, C=0.5, probabilistic=TRUE)
    AssertEqual(500, length(res))
    AssertEqual(50, mean(sapply(res, nrow)), tolerance=7, scale=1)
    AssertEqual(FALSE, any(sapply(res, function(r) any(duplicated(r)))))
}
