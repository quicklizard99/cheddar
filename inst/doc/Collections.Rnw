...
\begin{center}
<<fig=TRUE>>=
with(res, plot(pH, NumberOfNodes))
abline(model)
@
\end{center}
...
properties that we like and all of the power of \code{PlotNPS} is available. 
The example below plots trophic level as a function of 
...
As mentioned previously, collections cannot be modified directly because 
many checks for data consistency are run when collections are constructed 
(Section \ref{sec:basic_operations}). However, the function 
\code{CollectionApply} is provided to assist with modifying collections.
For example, with certain analyses it can be desirable to remove isolated nodes.
<<>>=
...
parameters. The following example reorders the nodes each community by body 
mass.
<<>>=
head(CollectionNPS(pHWebs))
...
\end{verbatim}
The parameter `class.behaviour' describes the behaviour for each R type. 
The parameter `column.behaviour' allows for deviations from `class.behaviour' 
for specific columns. 
The functions \code{MeanNaRm} and \code{JoinUnique} are provided by 
Cheddar. 
The default behaviour for numeric columns is therefore to compute a mean. 
This behaviour can be changed per-column, for example we can could have 
computed the median body mass.
<<>>=
