...
You should read the `CheddarIntro' and 'Plots' vignettes before reading this 
one. The `ImportExport' vignette shows how to get collections of communities 
into Cheddar.
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{lll}
      ...
      \midrule
        \code{Millstream} & The control and drought treatments from one replicate & \cite{LedgerEtAl2008Oecologia}        \\
                          & of a long-running study investigating how drought     & \cite{LedgerEtAl2011GlobChangeBiol}   \\
...
values into the collection. The following operations would raise errors if 
executed.
<<eval=FALSE>>=
...
communities. \code{CollectionCPS} takes a `properties' parameter that defines 
which properties will be returned. Just as with \code{CPS}, properties can be 
both `first-class` and computed. \code{CollectionCPS} is an powerful function 
that allows you to build up a \code{data.frame} of predictors and responses. 
...
The example below contains four computed properties.
<<>>=
...
which returns the total biomass in each class, which defaults to `category'.
<<>>=
CollectionCPS(pHWebs, c('pH',
                        S='NumberOfNodes',
                        L='NumberOfTrophicLinks', 
...
# Named properties
head(CollectionNPS(pHWebs, c('M','N',B='Biomass', 'Degree', 
                             Basal='IsBasalNode')))
@
...
to decide which which \code{Community}-level plot function to use. You can 
change this behaviour using the `plot.fn' parameter. 
...
We can use the general-purpose function \code{PlotNPS} to plot any node 
properties that we like. All of the power of \code{PlotNPS} is available. 
The example below plots trophic level as a function 
of $\log_{10}$-transformed body mass. Each plot has the same axis limits. We 
have turned off plotting of the food web and highlighting of cannibals.
...
\begin{center}
<<fig=TRUE>>=
plot(pHWebs, plot.fn=PlotTLPS, X='resource.Log10M', 
     Y='resource.PreyAveragedTrophicLevel', xlim=c(-14,6), ylim=c(1,4.2))
@
\end{center}

\subsection{Plot-per-collection}
\subsubsection{Distribution of body masses}
Distributions of body masses across habitat types \citep{DigelEtAl2011Oikos}. 
...
A plot similar to \cite{RiedeEtAl2010AER}, Fig.\ 1 (p 143) and 
\cite{BrownEtAl2011JAnimEcol}, Fig.\ 7 (p 891) but using different 
data.

\begin{center}
\SweaveOpts{width=2.6,height=8}
...
properties <- CollectionCPS(pHWebs, c(S='NumberOfNodes', 
                                      ...
                                      C='DirectedConnectance'))
par(mfrow=c(3,1))
plot(properties$S, properties$L)
plot(properties$S, properties$LinkageDensity)
plot(properties$S, properties$C)
@
\end{center}

\citep{LayerEtAl2010AER}, Fig.\ 4 (p 280). Cheddar's \code{pHWebs} dataset 
contains ten of the twenty food webs analysed by \cite{LayerEtAl2010AER} so 
the plot below is not an exact recreation of the published figure. 
<<>>=
properties <- CollectionCPS(pHWebs, c('pH', S='NumberOfNodes'))
plot(properties$pH, properties$S)
@
...
plot(1:length(n.links), n.links)
@
\end{center}

\section{Modifying communities}
For some analyses it can be desirable to remove isolated nodes.
<<>>=
...
\code{OrderCollection} allows to order collections by whatever properties 
...
into a new single community object.
<<>>=
a <- AggregateCommunities(pHWebs)

# Satisfy ourselves that each node has been included in the aggregated community
length(unique(CollectionNPS(pHWebs)$node))
NumberOfNodes(a)
@

The way that node, trophic-link and whole-community values are aggregated 
is controlled using two parameters.
\begin{verbatim}
...
\end{verbatim}
`class.behaviour' describes the behaviour for each R type. `column.behaviour' 
allows for deviations from `class.behaviour' for specific columns. 
The functions \code{MeanNaRm} and \code{JoinUnique} are provided by 
Cheddar. 
The default behaviour for numeric columns is therefore to computed a mean. This 
behaviour can be changed per-column, for example we can could have computed 
the median body mass.
<<>>=
b <- AggregateCommunities(pHWebs, column.behaviour=list(M=median))
@

\code{AggregateCommunitiesBy} aggregates by a single property, either 
first-class or computed, of the contained communities. Each food web 
in the pHWebs dataset has a different pH, so aggregating by pH would result in 
a collection of the same ten communities. The Duddon Pike Beck and Mosedal Beck 
communities share the same latitude and have pH values of 6.1 and 5.9 
respectively.
<<>>=
...
@
The aggregation of Duddon Pike Beck and Mosedal Beck has a pH of 6: the mean 
of the two pH values of the two communities.
...
\code{NPS} can properties such as body mass, $M$, numerical abundance, $N$', 
and trophic level. 
...
shown in a darker colour. See the \code{PlotNPS} function and the `Plots' 
