...
(Section \ref{sec:lump_trophic_species}).
...
\subsection{Lumping trophic species}
\label{sec:lump_trophic_species}
Species that share the same resources and consumers are said to be the same 
`trophic species' \citep{BriandAndCohen1984Nature, PimmEtAl1991Nature, 
WilliamsAndMartinez2000Nature}. The \code{LumpTrophicSpecies} function returns 
a new \code{Community} object with trophic species merged. 
<<>>=
NumberOfNodes(TL84)
NumberOfTrophicLinks(TL84)
...
NumberOfNodes(TL84.lumped)  # 22 species
NumberOfTrophicLinks(TL84.lumped)   # 71 links
@
