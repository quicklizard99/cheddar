...
A `trophic chain' is defined as a route from a basal node to a top-level node 
(Section \ref{sec:node_connectivity}). Nodes appear in a chain no more than 
once, so loops and cannibalism are ignored. The `length' of a chain is defined 
as the number of links in the chain, i.e.\ the number of nodes in the chain 
minus one. Cheddar provides two functions for examining trophic chains. 
The \code{TrophicChainsStats} function returns a list of simple 
statistics about trophic chains within a community.
<<>>=
...
@
This is particularly true for communities that contain a large number of 
trophic chains, such as the Skipwith Pond dataset, which has more than 
$10^5$ unique chains.
...
TL84.r <- RemoveNodes(TL84, to.remove, secondary=TRUE)
NumberOfNodes(TL84.r)
NumberOfTrophicLinks(TL84.r)

TL84.r <- RemoveNodes(TL84, to.remove, secondary=TRUE, cascade=TRUE)
NumberOfNodes(TL84.r)
NumberOfTrophicLinks(TL84.r)
@
