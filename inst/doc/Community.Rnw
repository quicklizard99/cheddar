...
trophic links into and out of that node. Cheddar contains three functions that 
...
A node in a community can be defined as falling into one of four categories 
...
(\code{BasalNode}) and ends with a top-level node (\code{TopLevelNode}) does 
not contain the same node more than once.
<<>>=
...
PlotPredationMatrix(res$reordered,
                    main=paste('Reordered - sum diet gaps', 
