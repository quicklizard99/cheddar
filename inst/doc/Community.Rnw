...
Certain analyses call for food-web nodes to be merged. One such example is 
the lumping together of nodes that share the same resources and consumers, 
so-called `trophic species' \citep{BriandAndCohen1984Nature, PimmEtAl1991Nature,
WilliamsAndMartinez2000Nature}. The \code{LumpTrophicSpecies} function returns 
a new \code{Community} object with trophic species lumped.
<<>>=
NumberOfNodes(TL84)
...
@
The `class.behaviour' and `column.behaviour' parameters control how values 
for lumped nodes are computed. The default for numeric values is to take the 
arithmetic mean. We could instead compute the median body mass.
<<>>=
TL84.lumped <- LumpNodes(TL84, lump, column.behaviour=list(M=median))
@
Or the maximum body mass.
<<>>=
TL84.lumped <- LumpNodes(TL84, lump, column.behaviour=list(M=max))
@
...
Ythan Estuary dataset contains two species that are split by life stage: 
\textit{Platichthys flesus} (european flounder) and 
\textit{Somateria mollissima} (common eider). The code fragment below shows how 
to lump these into a single node for each species. 
<<>>=
data(YthanEstuary)
