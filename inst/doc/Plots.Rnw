...
%\VignetteIndexEntry{Cheddar graphs}
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{cccll}
      ...
      \checkmark    &            & \checkmark & \code{PlotNTrophicPyramid}    & $\sum(\log_{10} (N))$ by trophic level                         \\
      \checkmark    & \checkmark & \checkmark & \code{PlotBRvBC} *            & $\log_{10} (B)$ of resource versus $\log_{10} (B)$ of consumer \\
      \checkmark    & \checkmark & \checkmark & \code{PlotBTrophicPyramid}    & $\sum(\log_{10} (B))$ by trophic level                         \\
...
and all of it's power and flexibility are available to all of the functions 
...
implications for \cite{ReumanEtAl2008EcolLett} and ecosystem-level 
\cite{BrownEtAl2004Ecology} theories. We can plot 
...
The \code{TL84} datasets include trophic links so the \code{PlotNPS} 
plots the food web and highlights cannibals by plotting them in a 
light-coloured circle with a dark border. We could turn off these behaviours.
...
\code{node.labels} also accepts a vector of labels, so we could use 
letters to label the nodes.
...
\code{PlotNPS} can plot any computed node property. For example, we can examine 
allometric degree distributions \citep{JonssonEtAl2005AER, OttoEtAl2007Nature, 
DigelEtAl2011Oikos}, which describe how species' numbers of trophic links 
scale with their log-transformed body masses.
...
\citep{JonssonEtAl2005AER} we can plot trophic level against log-transformed 
body mass. The \code{PreyAveragedTrophicLevel} uses the matrix-inversion method 
...
\cite{WilliamsAndMartinez2004AmNat}; see the help page for 
\code{TrophicLevels} for details.
\code{PlotNPS} accepts the range of graphical parameters provided by \R's 
plotting system, such as \code{ylim} and \code{main}. To directly compare 
...
It is common to plot some combination of $\log_{10}$-transformed body mass, 
$M$, numerical abundance, $N$, or biomass, $B$ (e.g.\ 
...
The \code{PlotRankNPS} function can log-transform the rank values shown on 
the x axis \citep{JonssonEtAl2005AER}.
...
\begin{center}
<<fig=TRUE>>=
PlotNPSDistribution(TL84, 'PreyAveragedTrophicLevel')
@
\end{center}
...
such as trophic species numbers. As with any \R plot, you can set directly set 
...
\begin{center}
<<fig=TRUE>>=
PlotNvM(TL84, col=1, pch=1, highlight.nodes=NULL)
@
\end{center}
...
We can improve on this by showing each level of taxonomic resolution in a 
specific colour by providing the `colour.spec' parameter.
...
The more complex example below uses the `kingdom' level of classification with 
a different colour and symbol.

\begin{center}
<<fig=TRUE>>=
symbol.spec = c(5, Bacteria=19, Plantae=1, Chromista=2, Protozoa=3, 
                Animalia=4)
colour.spec = c('black', Bacteria='pink2', Plantae='green2', 
                Chromista='blue3', Protozoa='yellow2', Animalia='red3')
PlotNvM(TL84, 
        symbol.by='kingdom', symbol.spec=symbol.spec, 
        colour.by='kingdom', colour.spec=colour.spec, 
        highlight.nodes=NULL)
legend("topright", legend=names(colour.spec), pch=symbol.spec, col=colour.spec)
@
\end{center}

You can use \code{pch=NA} to turn off symbols and just show the food web.
...
allow nodes individual nodes to be highlighted or lowlighted using the 
...
one or more properties, for example Ythan Estuary dataset has a large 
number of primary consumers that depend upon detritus, which has no $M$ or $N$. 
\code{PlotNPS} (and hence all the high-level functions that use it) places 
nodes without a property at the lowest extent of the relevant axis. The plot 
of $\log_{10}(N)$-versus=$\log_{10}(M)$ below shows detritus at the 
...
The location of the detritus node is determined by the extent of the data, 
or by the axes limits, if given.
...
The example below shows how \code{PlotNPS} behaves in four different cases - 
node lacks x property, node lacks y property, node lacks both x and y 
...
\code{PlotTLPS} (for Plot Trophic-Link PropertieS) plots one node property 
against another. Every Cheddar function that plots a point per trophic link 
delegates the job of plotting to \code{PlotTLPS} and all of it's power and 
flexibility are available to all of the functions discussed in this section.
...
links or nodes. If a property name begins with either `resource.' or 
`consumer.' then the remainder of the name is assumed to be a node property. 
For example, to plot $\log_{10}(M)$ of consumers against $\log_{10}(M)$ of 
consumers we can use the helper function \code{Log10M}.
...
We can make the plot more informative if the the limits of both the x and y 
axes are the same. Setting the `axes.limits.equal' parameter to \code{TRUE} 
instructs \code{PlotTLPS} to do just that, as shown below.
...
plotting. \code{PlotTLPS} uses the \code{category} node property, if 
present, of resources to decide plotting colours and symbols. Resources that 
are producers are shown by green circles, invertebrates by blue squares and 
vertebrate ectotherms by purple diamonds. This is explored more in 
Section \ref{sec:nps_colours and symbols}.

The example below plots a resource's trophic level against it's 
$\log_{10}$-transformed body mass.

\begin{center}
<<fig=TRUE>>=
PlotTLPS(TL84, 'resource.Log10M', 'resource.PreyAveragedTrophicLevel')
@
\end{center}

The convenience functions \code{PlotPredationMatrix}, \code{PlotMRvMC}, 
\code{PlotMCvMR}, \code{PlotNRvNC}, \code{PlotNCvNR}, \code{PlotBRvBC}, 
\code{PlotBCvBR} are `wrappers' around \code{PlotTLPS} that plot a 
predation matrix (a binary matrix with species shown in node order, starting 
at the top-left, points on the dashed line are cannibals) or 
$\log_{10}$-transformed body mass, $M$, numerical abundance, $N$, or biomass, 
$B$.
...
To plot the same data using the consumer's category to decide colours and 
symbols.
\begin{center}
<<fig=TRUE>>=
...
\section{Recreating figures from published articles}
...
PlotCommunityVCommunity(TL84, TL86, 'Log10N', xlim=c(-2,10), ylim=c(-2,10), 
                        xlab=~log[10]~(N~of~84), ylab=~log[10]~(N~of~86))
PlotCommunityVCommunity(TL84, TL86, 'Log10Biomass', 
                        xlim=c(-8,-2), ylim=c(-8,-2), 
                        xlab=~log[10]~(B~of~84), ylab=~log[10]~(B~of~86))
...
\subsection{\cite{LayerEtAl2011AER}}

\subsubsection{\cite{LayerEtAl2011AER}, Fig.\ 4 (p 280)}
...
<<fig=TRUE>>=
data(pHWebs)
cps <- CollectionCPS(pHWebs, c('pH', S='NumberOfNodes'))
plot(cps$pH, cps$S, xlab='pH', ylab='S')
m1 <- lm(cps$S ~ cps$pH)
abline(m1)
title(main='Layer et al. (2011) AER, Fig. 4 (p 280)', outer=TRUE)
@
\end{center}

\subsubsection{\cite{LayerEtAl2011AER}, Fig.\ 5 (p 281)}

\begin{center}
<<fig=TRUE>>=
plot(NA, NA, xlim=c(-15,10), ylim=c(-5,15), xlab=Log10MLabel(units='mg'), 
     ylab=Log10NLabel(units='m^-2'), type='n', main='')

# A list of list of lm models
models <- lapply(pHWebs, NvMLinearRegressions, class=NULL)
sapply(models, PlotLinearModels, col=1)
title(main='Layer et al. (2011) AER, Fig. 5 (p 281)', outer=TRUE)
@
\end{center}

\subsubsection{\cite{LayerEtAl2011AER}, Fig.\ 6 (p 282)}
The first four panels from this figure.
...
title(main='Layer et al. (2011) AER, Fig. 6 (p 282)', outer=TRUE)
@
\end{center}

\subsection{\cite{WoodwardEtAl2005AER}}
\subsubsection{\cite{WoodwardEtAl2005AER}, Fig.\ 4. (p 108)}
The top panel shows all trophic links. The bottom panel shows just links for 
...
par(mfrow=c(2,1))
PlotMvN(BroadstoneStream, show.nodes.as='labels', cex=1, label.cex=1, 
