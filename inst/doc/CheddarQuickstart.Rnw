...
detail in it's own vignette, listed in Section \ref{sec:further_reading}.
...
\code{NPS} assembles node properties into an \R \code{data.frame}. Let's look 
...
vertebrate ectotherms by purple diamonds. The \code{NvMLinearRegressions} and 
\code{PlotLinearModels} functions can be used to add regression lines 
through each of these three categories.
...
body mass against of the consumer against that of the resource for every 
trophic link in the community.
...
You should read the `Community' vignette. Plotting communities is covered in 
the `Plots' vignette. The `ImportExport' vignette shows how to get your 
community data in-to and out-of Cheddar. If you have data from several 
communities and are interested in seeing how community structure changes among 
them, read the `Collections' vignette. The `TriTrophic' vignette describes 
Cheddar's support for a novel analysis that simultaneously considers body 
mass, numerical abundance, and a food web \cite{CohenEtAl2009PNAS}.
