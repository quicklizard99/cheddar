...
@
\end{center}
The node shown in red is described as `Unclassified flagellates' in the 
\code{TL84} dataset.
...
\begin{center}
<<fig=TRUE>>=
symbol.spec = c(Bacteria=19, Plantae=1, Chromista=2, Protozoa=3, 
                Animalia=4, 5)
colour.spec = c(Bacteria='pink2', Plantae='green2', Chromista='blue3', 
                Protozoa='grey20', Animalia='red3', 'black')
PlotNvM(TL84, 
...
@
\end{center}
The black diamond is for the `Unclassified flagellates' node. This node 
does not have any associated classification and is matched by the unnamed 
values in \code{symbol.spec} and \code{colour.spec}.
...
\newpage
All of Cheddar's plot functions add axis ticks to the top and right of the 
plot by default. This can be turned off by using the 
