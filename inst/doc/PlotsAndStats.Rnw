...
`show.na' parameter is \code{TRUE}. The \code{PlotNvM}, \code{PlotMvN}, 
\code{PlotBvM} and \code{PlotMvB} functions all set `show.na' to \code{TRUE} by 
default. All other functions that delegate to \code{PlotNPS} set `show.na' to 
\code{FALSE} by default. The first of the $\log_{10}(N)$-versus-$\log_{10}(M)$ 
plots below show detritus at the bottom-left of the plot.
\SweaveOpts{width=8,height=4}
\setkeys{Gin}{width=\textwidth}
\begin{center}
<<fig=TRUE>>=
